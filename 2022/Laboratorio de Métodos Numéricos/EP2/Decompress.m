...
function resultado = derivate_fx(x,y,z,M,k,h)
  [a,b,c] = size(M);
  if(x==1) #borda superior
    resultado = (M(x,y,z)-M(x+k+1,y,z))/(h);
  elseif(x==a) #borda inferior
...
function resultado = derivate_fy(x,y,z,M,k,h)
  [a,b,c]=size(M);
  if(y==1) #borda esquerda
    resultado = (M(x,y+k+1,z)-M(x,y,z))/(h);
  elseif(y==b) #borda direita
...
function decompress(compressedImg, method, k, h)
    ...
    imgcomprimida=imread(compressedImg);
    [a, b, c]=size(imgcomprimida);

    imgdescomprimida = zeros(a + (a-1)*k, b + (b-1)*k, c);
    
    #cria a imagem decompressed
    for h=1:c
        for i=1:a
            for j=1:b
                      imgdescomprimida(1+(i-1)*(k+1), 1+(j-1)*(k+1), h) = imgcomprimida(i,j,h);
            endfor
        endfor
    endfor
      ...
      for t=1:nz
        ...
        while (i<nrow)
          ...
          while(j<ncol)
            ...
            #completa o quadrado pequeno (submatriz)
            for ii=i:i+k+1
              for jj=j:j+k+1
                if(M(ii,jj,t)==0)
...
decompress('compressed.png', 1, 1, 1);
